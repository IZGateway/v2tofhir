...
class StringTypeTests {
	...
	protected <P, T extends PrimitiveType<P>> void testFormattedInput(T actual, T expected, String expectedString) throws DataTypeException {
		if (actual != null) {
	...
	private static String[] validStrings = {
		"http://example.com/page#tag", // A URI
	...
	static List<String> validStrings() {
		List<String> l = Arrays.asList(validStrings).stream().flatMap(TestUtils::pad).collect(Collectors.toCollection(ArrayList::new));
		l.add(null);
		l.add(" ");
		return l;
	}
