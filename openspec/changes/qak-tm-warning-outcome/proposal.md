...
- Add an explicit `TM` case to `QAKParser.setIssueDetails()` (before the `NF`/`OK`/`default` group) that maps `QAK-2 = TM` to `OperationOutcome.issue.severity = warning` and `issue.code = too-costly` (HAPI R4 `IssueType.TOOCOSTLY`).
...
- Add a unit test asserting the `TM` → `warning` / `too-costly` mapping (v2tofhir currently has no QAK status→severity assertion).
...
### New Capabilities
- `qak-query-response-status`: Defines how `QAKParser` maps the HL7 v2 `QAK-2` query-response status (table 0208) to `OperationOutcome.issue` severity and code, including the newly-added `TM` ("too much data found") → `warning` / `too-costly` mapping and the preservation of the source coding in `issue.details`.
...
- **Tests:** New assertion(s) in the QAK/RSP parsing test coverage (test class suffixed `Tests`) verifying `TM` → `warning` / `too-costly` and that `OK`/`NF`/`AE`/`AR` are unaffected.
