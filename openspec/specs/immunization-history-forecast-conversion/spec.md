...
one observed instance, and it is logged as a pre-existing deviation to be fixed (map it to
`Immunization.fundingSource` and link it — see deviation 4 in the design of the
`fix-z42-history-forecast-split` change), so pinning today's behaviour in a test would cement the
defect.
